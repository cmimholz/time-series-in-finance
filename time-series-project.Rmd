...
In the world of stock trading, timing is everything—just like in dating. So, in true **Barney Stinson** fashion, we decided to test the legendary **3-Day Rule** on stocks, because if it works for love, why not try it for financial markets? According to **Barney**, you should wait three days before calling a girl. But what if you applied this rule to stocks? That’s exactly what we set out to find out!

In this project, we borrowed from **Barney’s 3-Day Rule**—waiting three days after a significant price drop before making a move. Just like Barney would give a girl three days to call back, we waited three trading days to see if a stock could "call back" with a recovery. Of course, we added a twist: instead of relying on gut feelings, we introduced predictive models like **ARIMA** to try and forecast whether that stock would return our affection—or leave us high and dry.

As we venture through this journey of "stock dating," we’ll explore whether the **3-Day Rule** can really lead to gains or if we’re just chasing after a market that’s playing hard to get. Spoiler alert: We may have had a few "bad dates" along the way, but who knows? With some fine-tuning, we might just find our perfect match.

So, grab your suits, put on your best game face, and let’s dive into this adventure where **Barney’s Rule** meets financial forecasting—because in trading, like in love, it’s all about taking risks and waiting for the right moment.

<br> 

![](images/3day.jpg){width="4in"} 
...
# Create a data frame for comparison of the three strategies
comparison_data <- data.frame(
  Strategy = c("Buy-and-Hold Benchmark", "Trading Strategy"),
  Return = c(average_benchmark_return, average_strategy_return)
)
...
# Bar plot to compare the returns of the three strategies with a custom color theme
ggplot(comparison_data, aes(x = Strategy, y = Return, fill = Strategy)) +
  ...
  scale_fill_manual(values = c("green", "red")) +  # Green for Buy-and-Hold, Red for Trading and ARIMA Strategy
  theme_minimal() +
...
# Set up a 2x2 grid of plots (4 plots, 2 per row)
par(mfrow = c(2, 2), mar = c(4, 4, 3, 1))  # Adjust margins for better readability
...
```{r arima_2, echo = FALSE}
# Set up a 2x2 grid of plots (4 plots, 2 per row)
par(mfrow = c(2, 2), mar = c(4, 4, 3, 1))  # Adjust margins for better readability

# Plot ARIMA forecasts for each of the given dates with more days and shaded confidence intervals
dates <- c("2022-07-26", "2023-03-13", "2023-03-15", "2024-08-02")
...
### **Key Insights**
1. The **SMI (Buy-and-Hold)** strategy outperformed both active strategies, suggesting that a passive approach may be more effective in certain market conditions than attempting to time the market.

2. Both the **Trading Strategy** and **ARIMA Strategy** yielded negative returns, with the Trading Strategy underperforming significantly. This suggests that short-term, reactive strategies may struggle to capture broader market trends.

3. The **ARIMA model**, though sophisticated, lacked the accuracy needed for consistent profits, reinforcing the idea that stock price forecasting is uncertain and simpler strategies may sometimes perform better.

4. Both strategies need refinement. Current approaches based on fixed thresholds and short-term decisions could benefit from a more dynamic, data-driven approach.
...
## **Usage of Gen AI in the Project**

Throughout this project, **Gen AI** played a crucial role in improving our analysis and decision-making for the strategy. It assisted in the development of forecasting models by suggesting improvements to existing algorithms, particularly in identifying and refining entry points for both the **Trading Strategy** and **ARIMA Strategy**. Gen AI also provided valuable insights into alternative strategies, offering recommendations on more advanced techniques, such as **Random Forest** and **LSTM**, which could potentially improve prediction accuracy, but where out of scope.

Moreover, **Gen AI** was used to optimize code and streamline the process of backtesting strategies, saving significant time and effort.

**THANK YOU!**

<br> 

![](images/wink.jpg){width="4in"} 
