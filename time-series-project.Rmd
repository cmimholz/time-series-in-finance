...
This section focuses on retrieving historical stock price data for companies listed in the Swiss Market Index (SMI) and the S&P 500 index. It defines the relevant stock symbols, uses a custom function to download data from Yahoo Finance, and handles errors gracefully during the process. The downloaded data is saved as an RDS file for future use, ensuring efficient access and avoiding redundant downloads.
...
This section introduces a function for visualizing the adjusted stock prices of specific companies over time. The function ensures that the required adjusted price data is available and generates a line chart displaying price trends with clear labels and formatting. 
...
# Example usage: Visualize adjusted prices for the Nestlé stock ("NESN.SW").
